    ...
    public function getDiscoListPreview(Request $request) {
        ...
        if ($request['Town'] == 'All') {
            $list = DB::table('Billing_Bills')
                ...
                ->whereRaw("Billing_Bills.AccountNumber NOT IN (SELECT AccountNumber FROM Billing_Excemptions WHERE ServicePeriod='" . $request['ServicePeriod'] . "')")
    ...
    public function printDisconnectionList($period, $area, $meterReader, $day) {
        if ($area == 'All') {
            ...
            $discoList = DB::table('Disconnection_NoticeHistory')
                ...
                ->whereRaw("Billing_Bills.AccountNumber NOT IN (SELECT AccountNumber FROM Billing_Excemptions WHERE ServicePeriod='" . $period . "')")
                ->select('Billing_ServiceAccounts.id as AccountNumber',
                    'Billing_ServiceAccounts.ServiceAccountName',
                    'Billing_ServiceAccounts.Purok',
