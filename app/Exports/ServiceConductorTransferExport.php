...
class ServiceConductorTransferExport implements ShouldAutoSize, WithColumnFormatting, WithEvents {
            ...
            AfterSheet::class => function(AfterSheet $event) { 
                ...
                foreach($this->data as $key => $value) {
                    if (($key % $dataRow) == 0) {
                        ...
                        $event->sheet->getStyle('A' . $row . ':M' . $row)
                            ->getFont()->setBold(true);          
                        $event->sheet->setCellValue('A' . $row, 'EXECUTED SERVICE CONDUCTOR TRANSFERS FROM ' . date('M d, Y', strtotime($this->from)) . ' TO ' . date('M d, Y', strtotime($this->to)));
                        ...
                        $event->sheet->getStyle('G' . $row)
                            ->getFont()->setBold(true);
                        $event->sheet->setCellValue('G' . $row, 'Old Meter Brand');
                        $event->sheet->getStyle('H' . $row)
                            ->getFont()->setBold(true);
                        $event->sheet->setCellValue('H' . $row, 'Old Meter Number');
                        $event->sheet->getStyle('I' . $row)
                            ->getFont()->setBold(true);
                        $event->sheet->setCellValue('I' . $row, 'Old Meter Reading');
                        $event->sheet->getStyle('J' . $row)
                            ->getFont()->setBold(true);
                        $event->sheet->setCellValue('J' . $row, 'New Meter Brand');
                        $event->sheet->getStyle('K' . $row)
                            ->getFont()->setBold(true);
                        $event->sheet->setCellValue('K' . $row, 'New Meter Number');
                        $event->sheet->getStyle('L' . $row)
                            ->getFont()->setBold(true);
                        $event->sheet->setCellValue('L' . $row, 'New Meter Reading');
                        $event->sheet->getStyle('M' . $row)
                            ->getFont()->setBold(true);
                        $event->sheet->setCellValue('M' . $row, 'Crew Assigned');
                        $event->sheet->getStyle('A' . $row . ':M' . $row)->applyFromArray($borderStyle);
                        ...
                        $event->sheet->setCellValue('G' . $row, $value->CurrentMeterBrand);
                        $event->sheet->setCellValue('H' . $row, $value->CurrentMeterNo);
                        $event->sheet->setCellValue('I' . $row, $value->CurrentMeterReading);
                        $event->sheet->setCellValue('J' . $row, $value->NewMeterBrand);
                        $event->sheet->setCellValue('K' . $row, $value->NewMeterNo);
                        $event->sheet->setCellValue('L' . $row, $value->NewMeterReading);
                        $event->sheet->setCellValue('M' . $row, $value->StationName);
                        $event->sheet->getStyle('A' . $row . ':M' . $row)->applyFromArray($borderStyle);
                        $row += 1;
