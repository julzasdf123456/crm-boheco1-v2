...
class MeterReplacementsExport implements ShouldAutoSize, WithColumnFormatting, WithEvents {
            ...
            AfterSheet::class => function(AfterSheet $event) { 
                ...
                foreach($this->data as $key => $value) {
                    if (($key % $dataRow) == 0) {
                        ...
                        $event->sheet->getStyle('A' . $row . ':M' . $row)
                            ->getFont()->setBold(true);          
                        $event->sheet->setCellValue('A' . $row, 'EXECUTED METER REPLACEMENTS FROM ' . date('M d, Y', strtotime($this->from)) . ' TO ' . date('M d, Y', strtotime($this->to)));
