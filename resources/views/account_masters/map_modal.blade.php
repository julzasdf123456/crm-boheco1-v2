@php
    if ($accountMaster->Item1 != null) {
        $gps = explode(',', $accountMaster->Item1);
        $lati = $gps[0];
        $longi = $gps[1];
    } else {
        $lati = null;
...
@push('page_scripts')
    ...
    <script>
        mapboxgl.accessToken = 'pk.eyJ1IjoianVsemxvcGV6IiwiYSI6ImNqZzJ5cWdsMjJid3Ayd2xsaHcwdGhheW8ifQ.BcTcaOXmXNLxdO3wfXaf5A';
            ...
            center: [parseFloat('{{ $longi }}'), parseFloat('{{ $lati }}')], // starting position [lng, lat] 
        ...
        map.on('load', () => {
            const el = document.createElement('div');
                el.className = 'marker';
                el.id = '{{ $accountMaster->AccountNumber }}';
                el.title = '{{ $accountMaster->ConsumerName }}'
            ...
            el.addEventListener('click', () => {
                        Swal.fire({
                            title : '{{ $accountMaster->ConsumerName }}',
